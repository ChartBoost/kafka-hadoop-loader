...
public class KafkaOutputFormat<K, V> extends FileOutputFormat<K, V> {
	...
	public RecordWriter<K, V> getRecordWriter(TaskAttemptContext context) throws IOException {
		...
		String ext = "";
		CompressionCodec snappyCodec = null;
		if (isCompressed) {
			Class<? extends CompressionCodec> codecClass = getOutputCompressorClass(context, SnappyCodec.class);
			snappyCodec = (CompressionCodec) ReflectionUtils.newInstance(codecClass, conf);
			ext = snappyCodec.getDefaultExtension();
		}
		final CompressionCodec codec = snappyCodec;
		...
		return new RecordWriter<K, V>() {
			...
			public void write(K key, V value) throws IOException {
				...
				String keyBasedPath = key.toString();

				RecordWriter<K, V> rw = this.recordWriters.get(keyBasedPath);
				try {
					if (rw == null) {
						Path file = new Path(((FileOutputCommitter) getOutputCommitter(taskContext)).getWorkPath(), getUniqueFile(taskContext, keyBasedPath
								+ "/" + taskContext.getJobID().toString().replace("job_", ""), extension));
