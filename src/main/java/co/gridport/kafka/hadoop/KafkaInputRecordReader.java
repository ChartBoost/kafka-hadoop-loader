...
public class KafkaInputRecordReader extends RecordReader<LongWritable, BytesWritable> {
	...
	public void initialize(InputSplit split, Configuration conf) throws IOException, InterruptedException {
		...
		log.info("Split {} Topic: {} Partition: {} Smallest: {} Largest: {} Starting: {}", new Object[] { this.split, topic, partition, smallestOffset,
	...
	public boolean nextKeyValue() throws IOException, InterruptedException {
		...
		//log.info("nextKeyValue messageAndOffset: " + messageAndOffset);
		if (messageAndOffset == null || watermark >= latestOffset) {
			return false;
		} else {
	...
	public void close() throws IOException {
		...
		if (numProcessedMessages > 0) {
			ZkUtils zk = new ZkUtils(conf.get("kafka.zk.connect"), conf.getInt("kafka.zk.sessiontimeout.ms", 10000), conf.getInt(
					"kafka.zk.connectiontimeout.ms", 10000));
	...
	private void resetWatermark(Long offset) {
		if (offset.equals(kafka.api.OffsetRequest.LatestTime()) || offset.equals(kafka.api.OffsetRequest.EarliestTime())) {
