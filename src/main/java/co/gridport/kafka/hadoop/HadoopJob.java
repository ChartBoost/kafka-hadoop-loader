...
public class HadoopJob extends Configured implements Tool {
	...
	public int run(String[] args) throws Exception {
		...
		conf.setBoolean("mapreduce.map.speculative", false); // For newer API

		String topic = "";

		if (cmd.hasOption("topics")) {
			topic = cmd.getOptionValue("topics");
			conf.set("kafka.topics", topic);
			Logger.getRootLogger().info("Using topics: " + conf.get("kafka.topics"));
		} else {
			printHelpAndExit(options);
		}

		boolean isCompressed = cmd.getOptionValue("compress-output", "on").equals("on");
		if (isCompressed) {
		...
		Path jarTarget = new Path(getClass().getProtectionDomain().getCodeSource().getLocation() + "../kafka-hadoop-loader.jar");
		...
		// Ready to launch
		Job job = new Job(jobConf, "kafka.hadoop.loader => " + topic);
		...
		KafkaOutputFormat.setCompressOutput(job, isCompressed);
	...
	private Options buildOptions() {
		...
		options.addOption(OptionBuilder.withArgName("zk").withLongOpt("zk-connect").hasArg().withDescription("Initial zk connection string for discovery")
				.create("z"));

		options.addOption(OptionBuilder.withArgName("offset").withLongOpt("offset-reset").hasArg()
				.withDescription("Reset offset to start or end of the stream e.g. 'smallest' or 'largest'").create("o"));

		options.addOption(OptionBuilder.withArgName("compression").withLongOpt("compress-output").hasArg().withDescription("Snappy output compression on|off")
				.create("c"));
		...
		options.addOption(OptionBuilder.withArgName("json|protobuf|avro").withLongOpt("input-format").hasArg()
				.withDescription("How are the input messages formatted in the topic").create("i"));
		return options;
	}
